...
vec4 LightContribution(Light light, vec3 surface_pos, vec3 eye_pos,
  ...
  vec3 diffuse_term = diffuse_coeff * light.color * sv_diffuse.rgb * attenuation;
  ...
  float specular_coeff = pow(max(0.0, dot(half_dir, normal)), sv_shininess);

  // Phong shading
//  vec3 reflection = normalize(-reflect(-surface_to_light, normal));
//  float specular_coeff = pow(max(0.0, dot(reflection, surface_to_eye)), 0.3 * sv_shininess);

  vec3 specular_term = specular_coeff * light.color * sv_specular.rgb * attenuation;
  specular_term = clamp(specular_term, 0.0, 1.0);

  return vec4(diffuse_term, sv_diffuse.a) +
         vec4(specular_term, sv_specular.a);
}
